...
func (m *Metadata) Marshal(writer *mp4.Writer) error {
	{ // `udta` - User Data Box
		context := mp4.Context{UnderUdta: true}

		if _, err := writer.StartBox(&mp4.BoxInfo{Type: mp4.BoxTypeUdta(), Context: context}); err != nil {
			return err
		}
		...
		{ // `meta` - Meta Box
			...
			if _, err := writer.StartBox(&mp4.BoxInfo{Type: mp4.BoxTypeMeta(), Context: context}); err != nil {
				return err
			}

			if _, err := mp4.Marshal(writer, &mp4.Meta{}, context); err != nil {
				return err
			}

			{ // `hldr` - Handler Box
				if _, err := writer.StartBox(&mp4.BoxInfo{Type: mp4.BoxTypeHdlr(), Context: context}); err != nil {
					return err
				}

				if _, err := mp4.Marshal(writer, &mp4.Hdlr{
			...
			{ // `ilst` - Ilst Box
				...
				if _, err := writer.StartBox(&mp4.BoxInfo{Type: mp4.BoxTypeIlst(), Context: context}); err != nil {
					return err
				}
				t := reflect.TypeOf(*m)
				v := reflect.ValueOf(*m)
				...
				for i := 0; i < t.NumField(); i++ {
					...
					if _, err := writer.StartBox(&mp4.BoxInfo{Type: boxType, Context: context}); err != nil {
						return err
					}

					{ // `data` - Data Box
						if _, err := writer.StartBox(&mp4.BoxInfo{Type: mp4.BoxTypeData(), Context: context}); err != nil {
							return err
						}
						...
						if _, err := mp4.Marshal(writer, &data, context); err != nil {
							return err
						}
			...
			{ // `free` - Free Box
				if _, err := writer.StartBox(&mp4.BoxInfo{Type: mp4.BoxTypeFree()}); err != nil {
					return err
				}
