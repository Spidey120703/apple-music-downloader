...
func buildFieldsStruct(t reflect.Type) []*field {
	...
	for i := 0; i < t.NumField(); i++ {
		ft := t.Field(i).Type
		tag, ok := t.Field(i).Tag.Lookup("bit")
		if !ok {
			continue
		}
...
func buildField(fieldName string, tag string) *field {
	...
	if _, contained := tagMap["string"]; contained {
		f.set(fieldString)
	}
...
func parseFieldTag(str string) map[string]string {
	...
	for _, e := range strings.Split(str, ",") {
		...
		if len(kv) == 2 {
			tag[strings.TrimSpace(kv[0])] = strings.TrimSpace(kv[1])
		} else {
...
func resolveFieldInstance[T IFieldObject](f *field, box T, parent reflect.Value) *fieldInstance {
	fi := fieldInstance{
		field: *f,
	}

	cfo, ok := parent.Addr().Interface().(IFieldObject)
	if ok {
		fi.cfo = cfo
	} else {
		fi.cfo = box
	...
	if fi.is(fieldSizeDynamic) {
		fi.size = fi.cfo.GetFieldSize(f.name)
	}

	if fi.is(fieldLengthDynamic) {
		fi.length = fi.cfo.GetFieldLength(f.name)
	}
