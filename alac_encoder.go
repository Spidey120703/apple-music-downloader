...
func (s *AlacInfo) Samples() (ret []fairplay.Sample) {
	for _, chunk := range s.chunks {
		for _, sample := range chunk.Samples {
							...
							{ // `alac` - Apple Lossless Audio Codec Encapsulation
								...
								_, err = mp4.Marshal(writer, &mp4.AudioSampleEntry{
									SampleEntry: mp4.SampleEntry{
										...
										DataReferenceIndex: alacInfo.enca.DataReferenceIndex,
									},
									EntryVersion: alacInfo.enca.EntryVersion,
									Reserved:     [3]uint16{},
									ChannelCount: alacInfo.enca.ChannelCount,
									SampleSize:   alacInfo.enca.SampleSize,
									SampleRate:   alacInfo.enca.SampleRate,
								}, context)
								if err != nil {
									return err
								}
								...
								{ // `alac` - Apple Lossless Audio Codec
									...
									_, err = mp4.Marshal(writer, alacInfo.alac, box.Context)
									if err != nil {
										return err
									}
